    ...
    await consumer.run({
        eachMessage: async ({ topic, message }) => {
            ...
            if (topic === 'orders') {
                ...
                console.log(`Cached order details for order ${orderEvent.orderId}`);
                return;
            }

            if (topic === 'payments') {
                const paymentEvent = JSON.parse(message.value.toString()) as PaymentSucceededEvent;
                const { orderId } = paymentEvent;
                
                // Retrieve the cached order data
                const orderDetails = orderDataCache.get(orderId);
                if (!orderDetails) {
                    console.error(`No order details found for order ${orderId}. Cannot reserve inventory.`);
                    return; // In a real app, publish a failure event
                }
                const { productId, quantity } = orderDetails.items[0];

                console.log(`Received PaymentSucceededEvent for order ${orderId}. Attempting to reserve ${quantity} of product ${productId}.`);

                const stockResult = await db.query('SELECT quantity FROM products WHERE product_id = $1', [productId]);
                
                if (stockResult.rows.length === 0 || stockResult.rows[0].quantity < quantity) {
                    console.error(`Not enough stock for product ${productId}.`);
                    // Here you would publish an InventoryOutOfStock event.
                    return;
                }
                
                await db.query('UPDATE products SET quantity = quantity - $1 WHERE product_id = $2', [quantity, productId]);
                ...
                await producer.send({
                    topic: 'inventory',
                    messages: [ { key: orderId, value: JSON.stringify(inventoryEvent), headers: { correlationId } } ],
                });
